<template>
  <div>
    ...
    <div v-if="item" class="detail-content">
      ...
      <div class="detail-card">
        ...
        <div class="card-meta" style="margin-top: 8px;">
          ...
          <span class="tag tag-type">{{ item.bedrooms }}</span>
          <span class="tag tag-region">📍 {{ item.region }} {{ item.location }}</span>
        </div>
        <div class="card-footer"><span class="date">📅 {{ item.available }}</span></div>
      </div>
    ...
    <div v-else class="empty-state"><div class="icon">📭</div><p>未找到该房源</p></div>
  </div>
</template>
